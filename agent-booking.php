...
if(isset($_REQUEST['tid']) && isset($_SESSION['agent_id']))
{
	$agent_id=$_SESSION['agent_id'];
    $Order_Id= base64_decode($_REQUEST['tid']);

     $oidquery=myslqi_query("SELECT cd.cust_id,cd.amount,cd.name,cd.email,cd.mobile,cb.* from customer_details cd LEFT JOIN customer_booking cb on cb.fk_cust_id=cd.cust_id 
     	WHERE transid=$Order_Id");
     $count=0;
	 while($oidrow=myslqi_fetch_array($oidquery))
	 {
	 $count++;
	 $oid=$oidrow['cust_id'];
	 $name=$oidrow['name'];
	...
	$details .= "</table>";
	    ...
	    $booking_dt = date('Y-m-d h:i:s');
           ...
           $update_query="UPDATE `customer_details` SET `pay_amount`='$amount',`payment_status` = 'Y',`payment date`='$booking_dt' WHERE `transid`=$Order_Id";
            myslqi_query($update_query);

         $update_agent_query=myslqi_query("UPDATE `agent-recharge` SET amount=amount-$amount WHERE `fk_agent_id`=$agent_id");   
