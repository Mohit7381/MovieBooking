...
if($_GET['url'])
{
$url=myslqi_real_escape_string($_GET['url']);
$url=explode('/', $url);
$city=ucfirst($url[0]);
$locationquery=myslqi_query("SELECT id FROM location where location='$city' ");
$location_row=myslqi_fetch_array($locationquery);
$location_id=$location_row['id'];

$moviesquery=myslqi_query("SELECT m.id,m.name,m.image,m.lang,ass.todate FROM assign_show ass LEFT JOIN movies m on m.id=ass.fk_movie_id WHERE ass.fk_location_id='$location_id' group by m.name order by ass.show_id desc");
			...
			<div class="wrap">
			 <div id="main" role="main">
			      ...
			      <ul id="tiles">
			        ...
			        <?php
			        $i=1;
			         while($row=myslqi_fetch_array($moviesquery))
			        {
