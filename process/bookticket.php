...
$result3 = myslqi_query($sql3);
$cnt3 = myslqi_num_rows($result3);
...
else
{
	...
	$check_query=myslqi_query($check_query);
 	$check_query_rows=myslqi_num_rows($check_query);
...
if($check_query_rows<=0)
{
	...
	for($j=0; $j<$forloop; $j++)
	{
		...
		{
			$booking_id = myslqi_insert_id();
		}
	}


	myslqi_query("UPDATE customer_details SET amount='$amount' where cust_id=$cust_id");
