...
echo $admin->menu();
$id=$_REQUEST['id'];
$query=myslqi_fetch_array(myslqi_query("SELECT * FROM theatres WHERE id='$id'"));
?>
...
<form action="process/edit_theater.php" method="POST">
	...
	<select id='th_location' name='th_location'>
		...
		<tr>
			...
			<td>
				<?php
					$status_query=myslqi_query("SELECT status,type,notifications from theatres WHERE id='$id'");
					$status_row=myslqi_fetch_array($status_query);
