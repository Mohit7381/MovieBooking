...
else
{
	...
	if($cnt > 0)
	{
        ...
        $sql1 = myslqi_query("select show_id from assign_show where fk_theater_id='$theater_id' and fk_location_id = '$location_id' and todate='0000-00-00'");
        $result1=myslqi_num_rows($sql1);
        $row1=myslqi_fetch_array($sql1);
        $showids=$row1['show_id'];
        if($result1>0)
        {
		  $moviesql=myslqi_query("INSERT INTO assign_show VALUES ('','$movie_id','$theater_id','$location_id','$morning','$matney','$firstshow','$secondshow','$addshowdate','$date')");
		  $impid=myslqi_insert_id();
          echo "<a href='assign-charges.php?show_id=".$impid."&theater_id=".$theater_id."'>Assign Charges</a>";
		  $moviedeletesql=myslqi_query("DELETE FROM assign_show where show_id='$showids'");
        }
        else
        {
        	
          $sql1 = myslqi_query("select * from assign_show where fk_theater_id='$theater_id' and fk_location_id = '$location_id' and fk_movie_id='$movie_id'");
          if(myslqi_num_rows($sql1)>0)
          {
          	$row1=myslqi_fetch_array($sql1);
         	$showid=$row1['show_id'];
            $moviesql=myslqi_query("UPDATE assign_show SET fk_movie_id='$movie_id',todate='$date' where show_id='$showids' ");  
          }
          else
          {
          	echo "hello";
