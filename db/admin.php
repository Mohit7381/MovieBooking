...
class admin
{
	...
	{
		$query=myslqi_query("select * from admin where id=1");
		while($row=myslqi_fetch_array($query))
		{
	...
	{
		$query=myslqi_query("select * from agent where username='$username' and password='$password'");
		while($row=myslqi_fetch_array($query))
		{
			if($username==$row['username'])
			{
	...
	{
		$query=myslqi_query("select * from theatre_login where username='$username' and password='$password'");
		while($row=myslqi_fetch_array($query))
		{
			if($username==$row['username'])
			{
	...
	{
		$query=myslqi_query("SELECT * FROM location ORDER BY id");
	...
	{
		$query=myslqi_query("SELECT * FROM movies ORDER BY id desc");
	...
	{
		$query=myslqi_query("SELECT m.name,s.name as song,s.downloadlink,s.id  FROM songs s left join movies m on m.id=s.fk_movieid ORDER BY s.id desc");
	...
	{
		$query=myslqi_query("SELECT * FROM slider");
	...
	{
		$query=myslqi_query("SELECT * FROM theatres ORDER BY id");
	...
	{
		$query=myslqi_query("SELECT a.*,l.location FROM agent a left join location l on l.id=a.fk_location_id ORDER BY a.id");
		$moviess= array();
		if(myslqi_num_rows($query)>0)
		{
			while($row=myslqi_fetch_array($query))
		{
			$testing=array();
	...
	{
		$query=myslqi_query("SELECT t.*,m.name FROM trailers t LEFT JOIN movies m on m.id=t.movie_id ORDER BY id");
	...
	{
		$query=myslqi_query("SELECT m.id,m.name,a.fk_location_id FROM assign_show a left join movies m on m.id=a.fk_movie_id WHERE a.fk_theater_id='$theater_id' AND a.todate>=CURDATE()");
