...
$conn=$db->createAndConnectDB($hostname, $username, $password, $database);
$frontend=new frontend();

//loation
$array=$frontend->getlocation($conn);
  $size=sizeof($array);
  $t_size=$size;
  $i=0;
  $t=0;
  ...
  if(@$_GET['location'])
{
$url=myslqi_real_escape_string($_GET['location']);
$url=explode('/', $url);
$locationname=$url[0];
$moviename=$url[1];
$url=$url[2];
...
<meta name="description" content="<?php echo $moviename; ?> Movie <?php echo $locationname; ?> Show Timings, Showtimes, Tickets &amp; Film shows in <?php echo $locationname; ?>. <?php echo $moviename; ?> Movie Release date &amp; Online Ticket Booking. Cinema Halls &amp; Theatre Tickets Rates &amp; Price " />
	...
	while($row=myslqi_fetch_array($result))
	{
		...
		$typesqlresult=myslqi_query($typesql);
		$typerow=myslqi_fetch_array($typesqlresult);
	...
	<?php
		...
		if(myslqi_num_rows($result1)>0)
		{
		while($row1 = myslqi_fetch_array($result1))
		{
			$id = $row1['show_id'];
